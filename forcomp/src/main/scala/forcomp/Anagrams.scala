...
object Anagrams {
  ...
  def wordOccurrences(w: Word): Occurrences = {
    w.toLowerCase.groupBy((char:Char)=>char).toMap.map{case (a,b)=> (a.toChar, b.length)}.toList.sorted
  }
  ...
  def combinations(occurrences: Occurrences): List[Occurrences] = {
    def combinationsAcc(occurrences: Occurrences, acc:List[Occurrences]):List[Occurrences]={
      if(occurrences.isEmpty) acc
        ...
        val (ch,tm)=occurrences.head
        val b=for{
                comb <- acc
                i<-1 to tm
              }yield (ch, i)::comb
        combinationsAcc(occurrences.tail, acc ++ b)
      }
    }
    combinationsAcc(occurrences.reverse, List[Occurrences](Nil))
  ...
  def subtract(x: Occurrences, y: Occurrences): Occurrences = {
    (y foldLeft SortedMap[Char,Int]() ++ x){ case (map, (ch, chFreq)) => {
      val updatedChValue = map(ch) - chFreq
      if (updatedChValue != 0) map.updated(ch, updatedChValue)
      else map - ch
    } }.toList
  }
