...
class NonEmpty(elem: Tweet, left: TweetSet, right: TweetSet) extends TweetSet {
  ...
  def descendingByRetweet: TweetList={
    def sortByRetweets(remSet:TweetSet):TweetList={
      if(remSet.isInstanceOf[Empty]){
        Nil
      }else{
        val mostRetweeted=remSet.mostRetweeted
        new Cons(mostRetweeted,sortByRetweets(remSet.remove(mostRetweeted)))
      }
    }
    sortByRetweets(this)
  }
...
object GoogleVsApple {
  ...
  def customCheck(tweetSet:TweetSet, list:List[String]):TweetSet= {
    def check(tweet: Tweet): Boolean = {
      list.exists(x => tweet.text.contains(x))
    }
    tweetSet.filter(check)
  }

  lazy val googleTweets: TweetSet = customCheck(TweetReader.allTweets, google)

  lazy val appleTweets: TweetSet = customCheck(TweetReader.allTweets, apple)
